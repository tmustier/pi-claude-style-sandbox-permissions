...
test("pipeline orders deny and safety asks before sandbox defaults", () => {
  assert.equal(
    action("git status --short", { config: { claudeDenyRules: ["Bash(git status:*)"] } }),
    "deny",
  );
...
test("dangerouslyDisableSandbox asks unless an allow rule already matches", () => {
  assert.deepEqual(
    decide("npm install", { sandboxAvailable: true, dangerouslyDisableSandbox: true }).action,
    "ask-unsandboxed",
  );
  assert.deepEqual(
...
test("Claude Code allow, ask, and excluded rules map to unsandboxed paths", () => {
  assert.equal(
    action("git push origin main", { config: { claudeAllowRules: ["Bash(git push:*)"] } }),
    "run-unsandboxed",
  );
  assert.equal(
    action("docker ps", { config: { claudeAskRules: ["Bash(docker:*)"] } }),
    "ask-unsandboxed",
  );
  assert.equal(
    action("docker ps", { config: { sandbox: { excludedCommands: ["Bash(docker:*)"] } } }),
    "ask-unsandboxed",
  );
});
...
test("default happy path runs sandboxed", () => {
  assert.equal(
    action("git add -- file.txt && git status --short | grep '^U' || true"),
    "run-sandboxed",
  );
});
...
test("no-srt and sandbox-disabled fallback only auto-runs read-only commands", () => {
  ...
  assert.equal(
    decide("git push origin main", { sandboxAvailable: false }).action,
    "ask-unsandboxed",
  );
  assert.equal(
    decide("git rm -f -- file.txt", { sandboxAvailable: false }).action,
    "ask-unsandboxed",
  );
  assert.equal(
    decide("git add -- file.txt && git status --short | grep '^U' || true", {
      sandboxAvailable: false,
    }).action,
    "ask-unsandboxed",
  );
  assert.equal(
    decide("git status --short > ./status.txt", { sandboxAvailable: false }).action,
    "ask-unsandboxed",
  );
  assert.equal(
